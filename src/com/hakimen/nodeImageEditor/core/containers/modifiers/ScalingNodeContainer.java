...
public class ScalingNodeContainer extends NodeContainer {
    ...
    public void update() {
        super.update();
        if (readerNodes.get(OUTPUT) instanceof ImageNode out && readerNodes.get(IMAGE) instanceof ImageNode image && readerNodes.get(WIDTH) instanceof NumberNode width && readerNodes.get(HEIGHT) instanceof NumberNode height) {
            var buff = new BufferedImage(width.getValue().intValue(),height.getValue().intValue(),BufferedImage.TYPE_INT_ARGB);
